...
def estimate():
    try:
        ...
        print(f"Received data - Make: {make}, Model: {model}, Year: {year}, Mileage Range: {mileage_range}, Fuel Type: {fuel_type}, Transmission: {transmission}")
        ...
        if not matched_cars.empty:
            ...
            # If no exact match go of range of mileage and make and model
            matched_cars = used_cars_df[
                (used_cars_df['Make'] == make) &
                (used_cars_df['Model'] == model) &
                (used_cars_df['mileage'] <= max_mileage + 50000) &
                (used_cars_df['mileage'] >= min_mileage - 50000) &
                (used_cars_df['fuel_type'] == fuel_type) &
                (used_cars_df['transmission'] == transmission)
            ]
            ...
            if not matched_cars.empty:
                ...
                # If still no match use just Make and Model and their mean estimates
                matched_cars = used_cars_df[
                    (used_cars_df['Make'] == make) &
                    (used_cars_df['Model'] == model)
                ]
                ...
                if not matched_cars.empty:
                    ...
                    # If exact match is not found fall back ona similar cars and there mean of estimate
                    matched_cars = used_cars_df[
                        (used_cars_df['Make'] == make)
                    ]
